package com.project.frontend.purchasesSystem.services.usecases;

import java.util.List;

import com.project.frontend.productsSystem.services.in.IGetAllProducts;
import com.project.frontend.purchasesSystem.models.Purchase;
import com.project.frontend.purchasesSystem.services.in.IGetAllPurchases;

import retrofit2.Call;
import retrofit2.Retrofit;
import retrofit2.converter.gson.GsonConverterFactory;

public class GetAllPurchases implements IGetAllPurchases{
     private final String BASE_URL = "http://localhost:8080";
    private final IGetAllPurchases service;

    public GetAllPurchases() {
        Retrofit retrofit = new Retrofit.Builder()
            .baseUrl(BASE_URL)
            .addConverterFactory(GsonConverterFactory.create())
            .build();

        this.service = retrofit.create(IGetAllPurchases.class);
    }

    @Override
    public Call<List<Purchase>> getAllPurchases() {
        return service.getAllPurchases();
    }
    
}
